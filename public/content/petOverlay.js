...
(() => {
  ...
  `;
    ...
    const send = (mode) => chrome.runtime.sendMessage({ type: "AI_ACTION", mode });
    bubble.querySelector("#proof")?.addEventListener("click", (e) => {
      e.stopPropagation();
      send("proofread");
    });
    bubble.querySelector("#rewrite")?.addEventListener("click", (e) => {
      e.stopPropagation();
      send("rewrite");
    });
    bubble.querySelector(".close")?.addEventListener("click", (e) => {
      e.stopPropagation();
